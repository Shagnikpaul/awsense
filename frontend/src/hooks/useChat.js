...
// ---------------------------------------------------------------------------
// Persistent, tamper-resistant session ID helpers
...
export function useChat() {
  ...
  // Load (or create) the persistent, verified session ID once on mount.
  useState(() => {
    loadOrCreateSessionId().then(setSessionId);
  });
  ...
  const clearConversation = useCallback(() => {
    setMessages([]);
    setIsRateLimited(false);
    // UUID is intentionally kept — it only resets when localStorage is cleared.
  }, []);
